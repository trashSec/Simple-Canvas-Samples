...
/**
 * Created by Анастасия on 23.01.2017.
 */
...
public class BallsView extends View {
    ...
    class MyTimer extends CountDownTimer {
        ...
        public void onTick(long millisUntilFinished) {
            for(int i=0;i<balls.length;i++) {
                if (balls[i].x + balls[i].vx < getWidth() - balls[i].r && balls[i].x + balls[i].vx > balls[i].r) balls[i].x += balls[i].vx;
                else balls[i].vx = -balls[i].vx;
                if (balls[i].y + balls[i].vy < getHeight() - balls[i].r && balls[i].y + balls[i].vy > balls[i].r) balls[i].y += balls[i].vy;
